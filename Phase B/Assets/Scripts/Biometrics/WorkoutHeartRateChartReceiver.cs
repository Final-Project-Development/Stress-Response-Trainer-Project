...
public class WorkoutHeartRateChartReceiver : MonoBehaviour
{
    ...
    [Tooltip("Preferred path: read packets from the shared UDPReceiver so the stress model and chart use the same socket.")]
    public UDPReceiver externalUdpReceiver;

    [Tooltip("Keep false when UDPReceiver listens on 5055. Set true only if this chart should open its own UDP socket.")]
    public bool receiveUdpDirectly = false;
    ...
    private bool subscribedToExternalReceiver;
    ...
    void Start()
    {
        ...
        if (externalUdpReceiver == null)
            externalUdpReceiver = FindFirstObjectByType<UDPReceiver>();

        InitializeChartUi();
        SubscribeExternalReceiver();
        StartUdpReceiver();
    ...
    public void SetExternalUdpReceiver(UDPReceiver receiver)
    {
        if (externalUdpReceiver == receiver && subscribedToExternalReceiver)
            return;

        UnsubscribeExternalReceiver();
        externalUdpReceiver = receiver;
        SubscribeExternalReceiver();
    }

    private void SubscribeExternalReceiver()
    {
        if (subscribedToExternalReceiver)
            return;
        ...
        externalUdpReceiver.OnSampleReceived += HandleExternalWatchSample;
        subscribedToExternalReceiver = true;
    }

    private void UnsubscribeExternalReceiver()
    {
        if (!subscribedToExternalReceiver || externalUdpReceiver == null)
            return;

        externalUdpReceiver.OnSampleReceived -= HandleExternalWatchSample;
        subscribedToExternalReceiver = false;
    }

    private void HandleExternalWatchSample(UDPReceiver.WatchSample sample)
    {
        if (sample == null)
            return;
        ...
        {
            HandleSessionStart(new HrTimelineMessage
            {
                type = sample.type,
                source = sample.source,
                device = sample.device,
                sessionId = string.IsNullOrWhiteSpace(sample.sessionId) ? "watch-session" : sample.sessionId,
        ...
        {
            HandleSessionEnd(new HrTimelineMessage
            {
                type = sample.type,
                source = sample.source,
                device = sample.device,
                sessionId = string.IsNullOrWhiteSpace(sample.sessionId) ? currentSessionId : sample.sessionId,
                sampleCount = sample.sampleCount,
                startedAt = sample.startedAt,
                endedAt = sample.endedAt,
                sentAt = sample.sentAt
        ...
        HandleHrSample(new HrTimelineMessage
        {
            type = "hr",
            mode = string.IsNullOrWhiteSpace(sample.mode) ? "live" : sample.mode,
            source = sample.source,
            device = sample.device,
    ...
    private static string BuildLiveSessionId(HrTimelineMessage message)
    {
        return BuildLiveSessionId(message != null ? message.source : null, message != null ? message.device : null);
    }
    ...
    private void HandleJson(string json)
    {
        ...
        if (message.bpm <= 0f && message.hr > 0f)
            message.bpm = message.hr;

        if (string.Equals(message.type, "hr", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(message.sessionId))
            message.sessionId = BuildLiveSessionId(message);

        if (string.Equals(message.type, "hr", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(message.measuredAt))
            message.measuredAt = DateTimeOffset.UtcNow.ToString("O");
