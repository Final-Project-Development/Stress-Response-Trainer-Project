...
public class PlayerInteract : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (VrGameplayInput.InteractPressed)
            TryInteract();
    }
    ...
    void TryInteract()
    {
        ...
        Ray ray = VrUiSupport.GetInteractionRay();
        ...
        if (gm == null || !gm.HasFirstAidKit())
        {
            if (TryPickFirstAidAlongRay(ray))
                return;
            if (cam != null && TryFirstAidKitFallbackWhenFacing(cam))
                return;
        }
        ...
        if (cam != null && TryWoundedInteractWhenFacing(cam))
            return;
